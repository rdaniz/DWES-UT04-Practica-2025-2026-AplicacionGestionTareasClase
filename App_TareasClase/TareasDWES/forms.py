...
class TareaGrupalForm(forms.ModelForm):
    ...
    # Campo extra para los colaboradores (varios alumnos/profesores)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Campo colaboradores: solo alumnos
        self.fields['colaboradores'].queryset = Usuario.objects.filter(rol=Usuario.ALUMNO)
        self.fields['creada_por'].required = True
    ...
    def clean(self):
        ...
        if not colaboradores or len(colaboradores) == 1:
            self.add_error('colaboradores', 'Debes seleccionar al menos dos colaboradores (alumnos) para una tarea grupal')

        return cleaned_data

    def save(self, commit=True):

        # Guardar primero la tarea
        tarea = super().save(commit=False)
        tarea.tipo = Tarea.GRUPAL  # Fuerza que sea grupal
        if commit:
            tarea.save()

        # Guardar detalle grupal y colaboradores

        # Busca en la base de datos un TareaGrupal cuyo campo tarea sea el que acabamos de guardar (tarea es la tarea recién creada).
        # Si no existe, lo crea automáticamente.
        # Devuelve una tupla (objeto, creado):
            # tarea_grupal: el objeto TareaGrupal que existe o que se acaba de crear.
            # created: True si se creó nuevo, False si ya existía.
        tarea_grupal, created = TareaGrupal.objects.get_or_create(tarea=tarea)

        tarea_grupal.colaboradores.set(self.cleaned_data['colaboradores'])
        if commit:
            tarea_grupal.save()
